...
class NASFPNCell(nn.Module):
    ...
    def __init__(self, out_channels: int, norm_type: str | None = "batch", block_config: list[BlockSpec] = None):
        super().__init__()
...
class NASFPN(BaseNeck):
    ...
    def __init__(
        ...
        if len(in_channels) < 3:
            raise ValueError(f"Expected at least 3 input levels, got {len(in_channels)}")
    ...
    def forward(self, features: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        if len(pyramid_features) != NUM_PYRAMID_NODES:
            raise ValueError(f"Expected {NUM_PYRAMID_NODES} pyramid levels, got {len(pyramid_features)}")
