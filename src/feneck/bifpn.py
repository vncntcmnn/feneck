...
class BiFPN(BaseNeck):
    ...
    def __init__(
        ...
        if self.num_input_levels > num_levels:
            raise ValueError(f"Too many input levels ({self.num_input_levels}) for total levels ({num_levels})")

        self.input_convs = nn.ModuleList([
            ConvNormLayer(in_ch, out_channels, kernel_size=1, norm_type=norm_type)
            if in_ch != out_channels else nn.Identity()
        ...
        self.bifpn_blocks = nn.ModuleList([
            BiFPNBlock(num_levels, out_channels, norm_type, use_fast_attention, epsilon)
...
if __name__ == "__main__":
    print("Testing BiFPN with simplified implementation...")

    # Test configurations
    test_configs = [
        {"in_channels": [512, 1024], "in_strides": [16, 32], "name": "C4+C5"},
        {"in_channels": [256, 512, 1024], "in_strides": [8, 16, 32], "name": "C3+C4+C5"},
        {"in_channels": [1024], "in_strides": [32], "name": "C5 only"},
    ]
    ...
    for config in test_configs:
        ...
        model = BiFPN(
            ...
            num_layers=2,
            norm_type="batch",
            use_fast_attention=False  # Keep simple for now
        )
        model.eval()  # Important for BatchNorm

        # Create inputs
        inputs = []
        for ch, stride in zip(config["in_channels"], config["in_strides"], strict=False):
            size = 256 // stride  # Reasonable sizes
            inp = torch.randn(2, ch, size, size)  # batch_size=2
            inputs.append(inp)
        ...
        try:
            ...
            print(f"✓ Success! {len(inputs)} inputs -> {len(outputs)} outputs")
