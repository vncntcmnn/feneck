...
class FeaturePyramidExtender(BaseNeck):
    ...
    def _build_higher_resolution_layers(self, in_channels: list[int], out_channels: int, norm_type: Literal["batch", "group"] | None):
        ...
        for i in range(self.extra_levels):
            ...
            if self.project_channels:
                source_channels = out_channels if i > 0 else out_channels
            else:
    ...
    def _build_lower_resolution_layers(self, in_channels: list[int], out_channels: int, norm_type: Literal["batch", "group"] | None):
    ...
    def _forward_with_higher_levels(self, features: list[torch.Tensor], projected_features: list[torch.Tensor]) -> list[torch.Tensor]:
    ...
    def _forward_with_lower_levels(self, features: list[torch.Tensor], projected_features: list[torch.Tensor]) -> list[torch.Tensor]:
